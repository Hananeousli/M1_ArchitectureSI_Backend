...
// GET api/<EtudiantController>/complet/5
        ...
        public async Task<ActionResult<EtudiantCompletDto>> GetEtudiantWithNotes(long id)
        {
            string role = "";
            string email = "";
            IUniversiteUser user = null;
            ...
            GetEtudiantCompletUseCase useCase = new GetEtudiantCompletUseCase(repositoryFactory);

            if (!useCase.IsAuthorized(role, user, id))
            {
                return Forbid();
            }
            ...
            try
            {
                Etudiant? etudiant = await useCase.ExecuteAsync(id);

                if (etudiant == null)
                {
                    return NotFound($"Etudiant avec l'ID {id} introuvable");
                }

                // Utiliser la méthode ToDto() pour convertir correctement
                EtudiantCompletDto dto = new EtudiantCompletDto().ToDto(etudiant);
